...
6. **质量自检**：生成后在结构化 JSON 中保留官方源覆盖率、链接可验证率、议题簇合并率、重复新闻率、低价值资讯占比等指标，供调试使用，不展示在阅读版日报中。
7. **结构化输出**：`reports/YYYY-MM-DD.json` 保留候选新闻、议题簇、来源等级、合并理由、置信度、质量评分和 benchmark 差距；`reports/YYYY-MM-DD.md` / `.html` 只保留读者需要看的正文与原文索引。
